...
rfc_feature_name: wharf-api-v5 # Use kebab-case
title: "RFC-0016: wharf-api-v5" # Update this with PR number/ID and feature name. Use leading zeros
...
Starting with v5, the wharf-api shall no longer accept nor return database
...
- `/pkg/models/database`: Database models, with [GORM](https://gorm.io/)
  ...
  These are equivalent to the models from v4, though with any
...
- Added path parameter `{providerId}` for value that was previously taken from
...
- This was not implemented before, but will be for v5. Goal is to remove the
  branch array from the project model by v6, and rely on this endpoint
  completely, as some users may have thousands of branches.
...
- Deprecated. Has not been moved, but instead planned to be removed completely.
...
- Added path parameter `{tokenId}` for value that was previously taken from
  the HTTP request body.
...
v6, and if I recall correctly that's mostly just the provider APIs that rely on
them to simplify their own code.

However, I have great plans for v6 with the [Hide providers behind API](https://iver-wharf.github.io/wharf-notes/hide-providers-behind-api)
...
Again, it's of utter importance that the old endpoints works exactly as before,
...
- Going with plural instead of singular endpoints. Though I have a personal
  preference for the singular word forms, especially if we encounter some of
  those that change drastically later, such as "person" vs "people".
...
- Does update-specific request models need their own models? Or shall we reuse
